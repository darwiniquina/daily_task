...
<template>
  <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex flex-col font-sans">
    <AppHeader />
    ...
    <main class="flex-1 container mx-auto px-6 py-10">
      <div class="max-w-6xl mx-auto space-y-10">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div>
            <h2 class="text-3xl font-black text-gray-900 mb-2">Your Journey</h2>
            <p class="text-gray-500 font-medium">Relive your productivity and see how far you've come.</p>
          </div>

          <div class="flex flex-wrap items-center gap-3">
             <!-- Quick Select Chips -->
             <div class="flex items-center bg-white p-1 rounded-2xl border shadow-sm">
                <Button 
                  variant="ghost" 
                  size="sm" 
             ...
             <div class="hidden sm:block h-6 w-px bg-gray-200"></div>
              ...
              <Popover>
                <PopoverTrigger as-child>
                  <Button variant="outline" class="justify-start text-left font-normal rounded-2xl h-12 bg-white border-gray-200 hover:bg-gray-50 hover:border-gray-300 transition-all shadow-sm group">
        ...
        <section class="space-y-4">
          ...
          <ContributionGraph 
            :activity-data="historyStore.activityData" 
            :selected-date="selectedDate"
            :start-date="historyStore.startDate"
            :end-date="historyStore.endDate"
        ...
        <section class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div :class="['md:col-span-1 p-6 rounded-[32px] border transition-all duration-500 shadow-sm flex flex-col items-center text-center justify-center bg-white', 
                historyStore.focusLoading ? 'opacity-50' : '']">
            <div :class="['h-16 w-16 rounded-2xl mb-4 flex items-center justify-center transition-colors duration-500', achievementLabel.bg]">
              <component :is="achievementLabel.icon" :class="['h-8 w-8', achievementLabel.color]" />
            </div>
            <h4 class="text-xl font-black text-gray-900 leading-tight">{{ achievementLabel.title }}</h4>
            <p class="text-sm font-bold text-gray-400 uppercase tracking-widest mt-1">{{ achievementLabel.desc }}</p>
          </div>

          <div class="bg-white p-6 rounded-[32px] border shadow-sm flex flex-col justify-center">
            ...
            <div class="flex items-baseline gap-2">
              <span class="text-5xl font-black text-gray-900 tracking-tighter">{{ formatTotalDuration(historyStore.selectedDateFocusSeconds) }}</span>
            </div>
          ...
          <div class="bg-white p-6 rounded-[32px] border shadow-sm flex flex-col justify-center">
            ...
            <div class="flex items-baseline gap-2">
              <span class="text-5xl font-black text-gray-900 tracking-tighter">{{ completionStats.percent }}%</span>
              <span class="text-sm font-black text-emerald-600">{{ completionStats.completed }}/{{ completionStats.total }}</span>
            </div>
            <div class="w-full h-2 bg-gray-100 rounded-full mt-4 overflow-hidden">
               <div class="h-full bg-emerald-500 rounded-full transition-all duration-1000" :style="{ width: `${completionStats.percent}%` }"></div>
            </div>
          </div>
        </section>
        ...
        <section class="space-y-6">
          <div class="flex items-center gap-3 border-t pt-10">
            <div class="h-10 w-10 bg-white rounded-2xl flex items-center justify-center shadow-sm border">
               <CalendarIcon class="h-5 w-5 text-blue-600" />
            </div>
            <div>
              <h3 class="text-xl font-bold text-gray-900">{{ formattedSelectedDate }}</h3>
              <p class="text-sm text-gray-500 font-bold uppercase tracking-wider">Historical Archive</p>
            </div>
          </div>

          <div v-if="historyStore.tasksLoading" class="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border border-dashed">
          ...
          <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div v-if="historyStore.selectedDateTasks.length === 0" class="col-span-full py-20 text-center bg-white rounded-3xl border shadow-sm border-dashed">
                ...
                <h4 class="font-black text-gray-900">No activity recorded</h4>
                <p class="text-gray-500 text-sm">You didn't have any tasks scheduled for this date.</p>
            </div>
            
            <TaskItem 
              v-for="task in historyStore.selectedDateTasks" 
              :key="task.id" 
              :task="task" 
              view-only
            />
